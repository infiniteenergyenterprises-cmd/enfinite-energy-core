              ...
              <thead>
                <tr className="border-b border-white/10">
                  {['Lead Details', 'Contact Info', 'Lead Type', 'Message Preview', 'Received', 'Status', 'Manage'].map(h => (
                    <th key={h} className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-white/30 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr key={lead.id} className="hover:bg-white/[0.03] transition-colors group">
                    ...
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center shrink-0 border border-white/10 group-hover:border-amber-500/30 transition-colors">
                          <span className="text-sm font-black text-white">{lead.name.charAt(0).toUpperCase()}</span>
                        </div>
                        <div>
                          <span className="font-bold text-sm text-white block">{lead.name}</span>
                    ...
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="space-y-2">
                        {lead.phone && (
                          <a href={`tel:${lead.phone}`} className="flex items-center gap-2 text-xs text-white/60 hover:text-amber-400 font-medium transition-colors">
                            <Phone className="w-3.5 h-3.5 opacity-70" />{lead.phone}
                          </a>
                        )}
                        {lead.email && (
                          <a href={`mailto:${lead.email}`} className="flex items-center gap-2 text-xs text-white/60 hover:text-amber-400 font-medium transition-colors">
                    ...
                    {/* Type */}
                    <td className="px-6 py-5 whitespace-nowrap">
                      <span className={`inline-flex items-center justify-center text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-lg border ${TYPE_COLORS[lead.type] || 'bg-white/5 text-white/60 border-white/10'}`}>
                        {lead.type}
                      </span>
                    </td>
                    {/* Message */}
                    <td className="px-6 py-5 max-w-[250px]">
                      <p className="text-xs text-white/50 line-clamp-2 leading-relaxed font-medium group-hover:text-white/70 transition-colors">{lead.message || '—'}</p>
                    </td>
                    {/* Time */}
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="flex items-center gap-2 text-xs text-white/50 font-semibold">
                        <Clock className="w-3.5 h-3.5 opacity-70" />{timeAgo(lead.createdAt)}
                      </div>
                    </td>
                    {/* Status Badge */}
                    <td className="px-6 py-5 whitespace-nowrap">
                      <span className={`inline-flex items-center justify-center text-[10px] font-black uppercase tracking-wider px-3 py-1.5 rounded-lg border ${STATUS_COLORS[lead.status] || STATUS_COLORS['NEW']}`}>
                        {lead.status}
                      </span>
                    </td>
                    {/* Action */}
                    <td className="px-6 py-5 whitespace-nowrap">
                      <div className="relative">
                        <select
                          ...
                          className="w-32 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs font-bold text-white focus:outline-none focus:border-amber-400 focus:ring-1 focus:ring-amber-400/50 cursor-pointer disabled:opacity-50 appearance-none hover:bg-white/10 transition-all"
                        >
                        ...
                        <ChevronRight className="w-3.5 h-3.5 text-white/30 absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none rotate-90" />
                      </div>
                    </td>
                  </tr>
                ))}
