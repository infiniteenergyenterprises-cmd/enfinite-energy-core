...
export default function SolutionsPage() {
  ...
  const projects = [
    { tag: "RESIDENTIAL", title: "5kW On-Grid System", loc: "Lucknow, Uttar Pradesh", cap: "5 kW", save: "₹62,000", img: "https://images.unsplash.com/photo-1613665813446-82a78c468a1d?w=800&q=80", desc: "A beautifully integrated residential solar system providing clean energy to a 4-bedroom villa. The system features premium monocrystalline panels and a smart inverter with mobile monitoring.", highlight: "Reduced monthly electricity bill by 92%" },
    { tag: "COMMERCIAL", title: "100kW Rooftop System", loc: "Indore, Madhya Pradesh", cap: "100 kW", save: "₹12,50,000", img: "https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&q=80", desc: "Commercial rooftop installation for a manufacturing unit. The system is designed to handle heavy day-time loads, significantly reducing the dependency on grid power and expensive diesel generators.", highlight: "ROI achieved in just 3.5 years" },
    ...
    { tag: "AGRICULTURE", title: "10HP Solar Water Pump", loc: "Nagaur, Rajasthan", cap: "10 HP", save: "1,20,000 Ltrs", img: "https://images.unsplash.com/photo-1592982537447-7440770cbfc9?w=800&q=80", desc: "An off-grid solar water pumping system empowering local farmers with reliable daytime irrigation. Features a high-efficiency submersible pump and automated controller.", highlight: "Zero diesel costs for irrigation" }
  ];

  const applications = [
    { 
      img: "https://images.unsplash.com/photo-1613665813446-82a78c468a1d?w=500&q=80", 
      title: "Homes & Villas",
      desc: "Transform your home with a customized solar rooftop solution. Reduce your electricity bills by up to 90% and increase your property value.",
      features: ["Net Metering Support", "Aesthetic Design", "25-Year Warranty", "Smart App Monitoring"]
    },
    { 
      img: "https://images.unsplash.com/photo-1497366216548-37526070297c?w=500&q=80", 
      title: "Offices & Buildings",
      desc: "Power your commercial spaces with smart solar installations. Benefit from tax savings, accelerated depreciation, and a sustainable brand image.",
      features: ["High ROI", "Tax Benefits", "Uninterrupted Power", "Scalable Systems"]
    },
    { 
      img: "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=500&q=80", 
      title: "Factories & Industries",
      desc: "Heavy-duty industrial solar plants designed to sustain massive loads. Hedge against rising commercial tariffs and achieve your CSR goals.",
      features: ["Heavy Load Support", "Grid Synchronization", "Custom Structures", "Zero Maintenance"]
    },
    { 
      img: "https://images.unsplash.com/photo-1592982537447-7440770cbfc9?w=500&q=80", 
      title: "Farms & Agriculture",
      desc: "Empowering farmers with off-grid solar water pumps. Eliminate expensive diesel costs and ensure reliable water supply during the day.",
      features: ["Zero Fuel Cost", "Government Subsidies", "Durable Pumps", "High Discharge"]
    },
    { 
      img: "https://images.unsplash.com/photo-1593941707882-a5bba14938c7?w=500&q=80", 
      title: "EV Charging Stations",
      desc: "The future is electric. We integrate solar power with EV charging stations to provide 100% clean, emission-free charging for all vehicles.",
      features: ["Clean Charging", "Future-proof", "Commercial Usage", "Fast Charging Support"]
    }
  ];
  ...
  return (
    <div className="bg-white min-h-screen text-[#0A192F]">
      {/* 1. Hero Banner */}
      <section className="relative pt-32 pb-32 px-4 sm:px-6 lg:px-8 bg-[#0A192F] text-white overflow-hidden">
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {applications.map((app, idx) => (
            <div key={idx} onClick={() => setSelectedApp(app)} className="relative h-64 rounded-xl overflow-hidden group cursor-pointer shadow-md border border-gray-100/10">
              ...
              <div className="absolute bottom-4 left-4 right-4 flex justify-between items-center text-white z-20">
                <span className="font-semibold text-sm drop-shadow-md">{app.title}</span>
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {projects.map((proj, idx) => (
            <div key={idx} onClick={() => setSelectedProject(proj)} className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden group cursor-pointer hover:shadow-xl transition-all hover:-translate-y-1">
              <div className="h-48 relative">
                ...
                <div className="absolute top-4 left-4 bg-amber-500 text-white text-[10px] font-bold px-2.5 py-1 rounded shadow-sm z-10">
                  {proj.tag}
                </div>
              </div>
              <div className="p-6 relative">
                <h4 className="font-bold text-lg mb-1 text-[#0A192F] group-hover:text-amber-500 transition-colors">{proj.title}</h4>
                <p className="text-gray-500 text-xs mb-5 flex items-center gap-1"><MapPin className="w-3 h-3" /> {proj.loc}</p>
                <div className="grid grid-cols-2 gap-4 mb-5 p-3 bg-gray-50 rounded-lg border border-gray-100/50">
                  <div>
                    <p className="text-gray-400 text-[10px] uppercase font-bold tracking-wider mb-1">Capacity</p>
                    <p className="font-bold text-sm text-[#0A192F]">{proj.cap}</p>
                  </div>
                  <div>
                    <p className="text-gray-400 text-[10px] uppercase font-bold tracking-wider mb-1">{proj.tag === 'AGRICULTURE' ? 'Daily Output' : 'Annual Savings'}</p>
                    <p className="font-bold text-sm text-[#0A192F]">{proj.save}</p>
                  </div>
                </div>
                <div className="text-amber-500 font-semibold text-sm flex items-center gap-1 group-hover:text-amber-600">View Details <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></div>
              </div>
            </div>
          ))}
        </div>
