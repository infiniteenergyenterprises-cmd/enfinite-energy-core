...
export default function OurWorkPage() {
  ...
  const featuredProjects = [
    { tag: 'Commercial', color: 'bg-blue-500', title: '100kW Commercial Project', loc: 'Noida, Uttar Pradesh', cap: '100 kW', cost: '₹12.5 Lakhs', save: '₹1.5 Lakhs/yr', img: 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&q=80' },
    { tag: 'Industrial', color: 'bg-purple-500', title: '500kW Industrial Plant', loc: 'Surat, Gujarat', cap: '500 kW', cost: '₹40 Lakhs', save: '₹6.5 Lakhs/yr', img: 'https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?w=800&q=80' },
    { tag: 'Agriculture', color: 'bg-green-500', title: '20HP Solar Water Pump', loc: 'Hisar, Haryana', cap: '20 HP', cost: '₹2.85 Lakhs', save: '₹1.2 Lakhs/yr', img: 'https://images.unsplash.com/photo-1592982537447-7440770cbfc9?w=800&q=80' },
    { tag: 'Government', color: 'bg-orange-500', title: '250kW Govt. Building', loc: 'Bhopal, Madhya Pradesh', cap: '250 kW', cost: '₹28 Lakhs', save: '₹3.5 Lakhs/yr', img: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=800&q=80' },
    { tag: 'Residential', color: 'bg-teal-500', title: '10kW Premium Home Solar', loc: 'Jaipur, Rajasthan', cap: '10 kW', cost: '₹4.5 Lakhs', save: '₹90,000/yr', img: '/17.png' },
    { tag: 'Institutional', color: 'bg-pink-500', title: '50kW University Campus', loc: 'Pune, Maharashtra', cap: '50 kW', cost: '₹22 Lakhs', save: '₹4.2 Lakhs/yr', img: '/hero-bg.png' }
  ];
  ...
  let filteredProjects = featuredProjects.filter(proj => {
    // 1. Category Filter
    if (activeFilter !== 'All Projects' && proj.tag !== activeFilter) {
      return false;
    }
    // 2. Search Filter
    if (searchQuery.trim() !== '') {
      const query = searchQuery.toLowerCase();
      if (!proj.title.toLowerCase().includes(query) && !proj.loc.toLowerCase().includes(query)) {
        return false;
      }
    }
    return true;
  ...
  return (
    <div className="bg-gray-50 min-h-screen font-sans text-gray-900">
      ...
      {/* 1. Hero Section */}
      <section className="relative pt-24 pb-32 px-4 sm:px-6 lg:px-8 bg-[#0B1120] border-b border-gray-200">
      ...
      {/* 2. Filter Bar */}
      <section id="featured-projects" className="pt-20 pb-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        >
          ...
          {filteredProjects.length > 0 ? (
            ...
            <div key={idx} onClick={() => setSelectedProject(proj)} className="w-[85vw] sm:w-[320px] shrink-0 snap-start bg-white rounded-xl overflow-hidden border border-gray-200 hover:border-emerald-500/50 hover:shadow-lg transition-all group cursor-pointer">
              ...
              <div className="p-4">
                <h3 className="font-bold text-gray-900 text-[13px] mb-1 group-hover:text-emerald-600 transition-colors truncate">{proj.title}</h3>
                <p className="text-gray-500 text-[10px] mb-4 flex items-center gap-1"><MapPin className="w-2.5 h-2.5" /> {proj.loc}</p>
                
                <div className="grid grid-cols-3 gap-1 text-center mb-4 border-t border-gray-100 pt-3">
                  <div>
                    <p className="text-[10px] font-bold text-gray-900">{proj.cap}</p>
                    <p className="text-[8px] text-gray-500 uppercase tracking-wide">Capacity</p>
                  </div>
                  <div className="border-l border-gray-100">
                    <p className="text-[10px] font-bold text-gray-900">{proj.cost}</p>
                    <p className="text-[8px] text-gray-500 uppercase tracking-wide">Project Cost</p>
                  </div>
                  <div className="border-l border-gray-100">
                    <p className="text-[10px] font-bold text-emerald-600">{proj.save}</p>
                    <p className="text-[8px] text-gray-500 uppercase tracking-wide">Savings</p>
                  </div>
                </div>
                
                <div className="text-emerald-600 text-[11px] font-bold flex items-center gap-1 group-hover:text-emerald-700">View Details <ChevronRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" /></div>
              </div>
            </div>
            ))
          ) : (
      ...
      {/* 8. Bottom CTA */}
      <section className="py-8 px-4 sm:px-6 lg:px-8 bg-[#0B1120]">
