...
export default function CompanyPage() {
  ...
  return (
    <div className="bg-white min-h-screen text-gray-900">
      ...
      {/* ══ HERO ══ */}
      <section className="relative pt-24 pb-12 bg-[#0B1E3D] overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            ...
            <div>
              ...
              <h1 className="text-4xl md:text-5xl font-black text-white leading-tight mb-4">
                Powering India's<br /><span className="text-[#F5A623]">Sustainable Future</span>
              </h1>
      ...
      {/* ══ IMPACT STATS BAR ══ */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-5 relative z-10 mb-8">
        <div className="bg-[#0B1E3D] border border-white/10 rounded-xl px-4 py-3 shadow-2xl">
          <div className="grid grid-cols-6 gap-1 md:gap-3">
            {impactStats.map((s,i) => (
              <div key={i} onClick={() => handleOpenDetail(s.label, s.desc, `${s.val} Achieved`, 'stat')} className={`rounded-md border p-0.5 md:px-2 md:py-2.5 text-center hover:scale-[1.03] transition-all group cursor-pointer ${s.color}`}>
                <div className="flex justify-center mb-0.5 md:mb-1.5 group-hover:scale-105 transition-transform">
                  {React.cloneElement(s.icon, { className: "w-3 h-3 md:w-5 md:h-5 text-current" })}
                </div>
                <p className="text-[9px] sm:text-base font-black text-white leading-none">
                  <span className="hidden sm:inline">{s.val}</span>
                  <span className="inline sm:hidden">{s.shortVal}</span>
                </p>
                <p className="text-[5.5px] sm:text-[10px] text-white/50 mt-0.5 leading-none font-medium uppercase tracking-wider">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-[#F5A623] text-[10px] font-black uppercase tracking-widest leading-none">Who We Are</p>
            </div>
      ...
      {/* ══ OUR JOURNEY ══ */}
      <section id="journey" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-12">
      ...
      {/* ══ LEADERSHIP TEAM ══ */}
      <section id="leadership" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 scroll-mt-24">
          ...
          {team.map((t,i)=>(
            <div key={i} onClick={() => handleOpenDetail(t.name, t.bio, `${t.role} (${t.exp})`, 'member')} className="bg-white border border-gray-100 rounded-2xl p-5 text-center shadow-sm hover:shadow-md hover:-translate-y-1 transition-all group cursor-pointer w-[245px] md:w-auto shrink-0 snap-start">
              ...
              <div className="flex justify-center gap-2 mt-3" onClick={(e)=>e.stopPropagation()}>
                ...
                <a href={`mailto:info@enfiniteenergy.com?subject=Inquiry to ${t.name}`} className="w-7 h-7 bg-gray-50 border border-gray-100 rounded-lg flex items-center justify-center text-gray-500 hover:bg-gray-100 transition-all">
