...
function SolutionDetailContent({ slug, solution }: { slug: string; solution: typeof solutionsData[keyof typeof solutionsData] }) {
  const { openModal } = useLeadModal();
  const KEY     = `SOL_DETAIL_${slug.toUpperCase().replace('-', '_')}`;

  // Fetch admin-saved content for this solution
  const cms = usePageContent(KEY, {
    title:       solution.title,
    imageUrl:    solution.image,
    description: '{}',
  });

  // Parse the JSON-encoded extra fields saved by admin
  let extra: { subtitle?: string; description?: string; benefits?: string[] } = {};
  try { extra = JSON.parse(cms.description || '{}'); } catch {}

  const title       = cms.title    || solution.title;
  const subtitle    = extra.subtitle    || solution.subtitle;
  const description = extra.description || solution.description;
  const image       = cms.imageUrl  || solution.image;
  const benefits    = (extra.benefits && extra.benefits.length === 4)
                        ? extra.benefits
                        : solution.benefits;
  ...
  return (
    <div className="bg-[#0A192F] min-h-screen pt-28">
      <style>{`
        ...
        .animate-slide-right { animation: slideRight 1.5s infinite ease-in-out; }
        @keyframes slideDown {
          0% { transform: translateY(0); opacity: 0.3; }
          50% { transform: translateY(12px); opacity: 1; }
          100% { transform: translateY(0); opacity: 0.3; }
        }
        .animate-slide-down { animation: slideDown 1.5s infinite ease-in-out; }
      `}</style>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center mb-8">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-amber-900/30 border border-amber-500/30 text-amber-400 font-bold text-xs uppercase tracking-wider mb-4">
          <Zap className="w-4 h-4" /><span>Proven Process</span>
        </div>
      ...
      <section className="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto bg-white rounded-t-3xl mt-4 relative z-10">
        <div className="flex flex-col lg:flex-row gap-12 items-center">
          <div className="lg:w-1/2 relative group">
            ...
            <img src={image} alt={title} className="relative w-full h-auto object-cover rounded-[2rem] shadow-2xl z-10" />
          ...
          <div className="lg:w-1/2">
            ...
            <p className="text-gray-600 text-[17px] leading-relaxed mb-8">{description}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
                {benefits.map((benefit, idx) => (
                  <div key={idx} className="flex items-start gap-3 bg-[#0A192F]/90 backdrop-blur-xl p-5 rounded-2xl border border-white/20 hover:border-amber-400/50 shadow-xl hover:shadow-[0_8px_30px_rgba(249,115,22,0.3)] hover:-translate-y-1 transition-all duration-300 group">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-amber-400 to-amber-600 flex items-center justify-center shrink-0 shadow-[0_0_15px_rgba(249,115,22,0.5)] group-hover:scale-110 transition-transform">
      ...
      <section className="py-10 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto bg-gradient-to-br from-[#0A192F] to-[#112240] rounded-[2rem] p-10 md:p-16 shadow-2xl relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-10 border border-white/10">
          ...
          <div className="relative z-10 md:w-2/3 text-center md:text-left">
            ...
            <p className="text-gray-300 text-lg leading-relaxed">Take the first step towards a sustainable future. Fill out the form and our experts will contact you with a customized plan.</p>
          </div>
          <div className="relative z-10 md:w-1/3 flex justify-center md:justify-end">
            <button onClick={() => openModal(solution.formType)}
...
export default function SolutionDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <SolutionDetailContent slug={slug} solution={solution!} />;
}
