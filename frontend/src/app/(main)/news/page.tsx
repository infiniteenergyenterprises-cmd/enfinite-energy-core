...
export default function NewsAndEventsPage() {
  ...
  const [heroImg,   setHeroImg]   = useState('/18.png');
  const [heroTitle, setHeroTitle] = useState('News & Events');
  const [heroDesc,  setHeroDesc]  = useState('Stay updated with the latest solar industry news, company updates and upcoming events.');

  // Static fallback company news
  const fallbackCompanyNews = [
    { category: "New Project", source: "Enfinite Energy", time: "May 25, 2025", title: "500kW Solar Plant Commissioned in Lucknow", summary: "Another milestone towards our mission of clean energy for all.", image: "https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&q=80", views: "2.1K", link: "/news/1" },
    { category: "Award", source: "Enfinite Energy", time: "May 20, 2025", title: "Enfinite Energy Wins Green Excellence Award 2025", summary: "Honored for outstanding contribution to renewable energy sector.", image: "/17.png", views: "3.4K", link: "/news/2" },
    { category: "Partnership", source: "Enfinite Energy", time: "May 18, 2025", title: "Strategic Partnership with Leading Solar Inverter Brand", summary: "Partnership to deliver advanced solar solutions across India.", image: "https://images.unsplash.com/photo-1563986768494-4dee2763ff3f?w=800&q=80", views: "1.8K", link: "/news/3" },
    { category: "CSR", source: "Enfinite Energy", time: "May 15, 2025", title: "Tree Plantation Drive Across 5 States", summary: "Our commitment towards a greener and sustainable future.", image: "https://images.unsplash.com/photo-1542601906990-b4d3fb778b09?w=800&q=80", views: "5.2K", link: "/news/4" },
  ];

  // Static fallback events
  const fallbackEvents = [
    { day: "15", month: "Jun", year: "2025", title: "Solar India Expo 2025", location: "Pragati Maidan, New Delhi", desc: "Asia's largest solar energy exhibition and conference featuring the latest innovations in renewable tech.", image: "https://images.unsplash.com/photo-1559336197-f584afb72bf8?w=800&q=80", time: "09:00 AM - 06:00 PM" },
    { day: "08", month: "Jul", year: "2025", title: "Future of Solar Energy in India", location: "Virtual Event (Zoom)", desc: "Join our expert panel to learn about emerging trends, government policies, and business opportunities.", image: "https://images.unsplash.com/photo-1515187029135-18ee286d815b?w=800&q=80", time: "03:00 PM - 05:00 PM" },
    { day: "20", month: "Jul", year: "2025", title: "Rooftop Solar System Design Workshop", location: "Enfinite HQ, Lucknow", desc: "A comprehensive hands-on workshop to learn rooftop solar design, estimation, and deployment strategies.", image: "https://images.unsplash.com/photo-1509391366360-1e97f52ce23b?w=800&q=80", time: "10:00 AM - 04:00 PM" },
  ];
  ...
  useEffect(() => {
    async function fetchAll() {
      try {
        ...
        else setCompanyNews(fallbackCompanyNews);
        if (evtRes.status === 'fulfilled' && evtRes.value.status === 'success' && evtRes.value.data.length > 0) setEvents(evtRes.value.data);
        else setEvents(fallbackEvents);
        if (cmsRes.status === 'fulfilled' && cmsRes.value.map?.['NEWS_HERO']) {
          const h = cmsRes.value.map['NEWS_HERO'];
          if (h.imageUrl)    setHeroImg(h.imageUrl);
          if (h.title)       setHeroTitle(h.title);
          if (h.description) setHeroDesc(h.description);
        }
        if (evtRes.status === 'fulfilled' && evtRes.value.status === 'success' && evtRes.value.data.length > 0) setEvents(evtRes.value.data);
        else setEvents(fallbackEvents);
      } catch { setCompanyNews(fallbackCompanyNews); setEvents(fallbackEvents); }
  ...
  // latestNews comes from liveNews state (RSS feeds)
  ...
  return (
    <div className="bg-gray-50 min-h-screen pt-8">
      ...
      <section className="relative bg-[#0B1120] pt-8 pb-8 overflow-visible border-b border-gray-200 min-h-[280px]">
        ...
        {/* Hero text */}
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-4 lg:px-6 pt-4">
        ...
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 z-20 w-full max-w-4xl px-4 sm:px-4 lg:px-6">
      ...
      <div className="sticky top-16 z-40 bg-white border-b border-gray-200 shadow-sm mt-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-4 lg:px-6 flex overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-4 lg:px-6 mb-10">
      ...
      {/* Final CTA Strip */}
      <section className="max-w-7xl mx-auto px-4 sm:px-4 lg:px-6 mb-8">
