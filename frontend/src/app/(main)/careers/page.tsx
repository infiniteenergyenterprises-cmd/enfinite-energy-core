...
export default function CareersPage() {
  ...
  const [heroTitle, setHeroTitle] = useState('Build Your Future\nwith Enfinite Energy');
  const [heroDesc, setHeroDesc]   = useState("Join our passionate team and be a part of India's clean energy revolution. We are building a greener, brighter tomorrow — together.");
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/content')
      ...
      .then(d => {
        const map = d.map || {};
        if (map['CAREERS_HERO']) {
          if (map['CAREERS_HERO'].imageUrl) setHeroImg(map['CAREERS_HERO'].imageUrl);
          if (map['CAREERS_HERO'].title)    setHeroTitle(map['CAREERS_HERO'].title);
          if (map['CAREERS_HERO'].description) setHeroDesc(map['CAREERS_HERO'].description);
        }
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <div className="bg-white min-h-screen text-gray-900">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            <div>
              ...
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-black leading-tight text-white mb-4">
                {heroTitle.split('\n').map((line, i) => (
                  <span key={i}>{i === 1 ? <><span className="text-[#F5A623]">{line}</span></> : line}{i < heroTitle.split('\n').length - 1 && <br />}</span>
                ))}
              </h1>
      ...
      {/* ══════════════════════ SEARCH BAR ══════════════════════ */}
      <section className="max-w-7xl mx-auto px-4 sm:px-4 lg:px-6 -mt-6 mb-8 relative z-10">
      ...
      {/* ══════════════════════ OPENINGS (FULL WIDTH) ══════════════════════ */}
      <section id="openings" className="max-w-7xl mx-auto px-4 sm:px-4 lg:px-6 mb-10">
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-400">No positions match your filters.</div>
        ) : (
